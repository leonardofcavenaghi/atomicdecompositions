...
def test_product_p1xp2():
    """P^1 × P^2 = A1xA2 algebra: dimension, Schubert basis, and SQM shape."""
    X = FlagVariety('A1xA2', [1, 2])   # P^1 × P^2
    assert X.dimension == 3             # dim = 1 + 2
    assert len(X.classes) == 6         # |W/P| = 2 * 3
    # SQM should produce a 6x6 matrix
    M, Gr, idx = X.small_quantum_multiplication()
    n = len(idx)
    assert len(M) == n and all(len(row) == n for row in M)


def test_flag_a3_two_kept_nodes():
    """Fl(1,2,4) = FlagVariety('A3', [1,2]): dim 5, classes, SQM shape."""
    X = FlagVariety('A3', [1, 2])   # Fl(1,2 in C^4): dim = 5
    assert X.dimension == 5
    # |W/P| for A3/P_{1,2}: |W| = 24, |W_P| = 1 (both kept) -> 24 classes
    # Actually the subgroup W_P is generated by s_3, so |W_P| = 2 -> 12 classes
    assert len(X.classes) > 1
    # Fano index of Fl(1,2,4): c1 = alpha_1 + 2*alpha_2 + alpha_3 in omega basis
    fano, betas = X.fano_index_and_betas()
    assert fano >= 1
    # SQM should be a square matrix of size len(X.classes)
    M, Gr, idx = X.small_quantum_multiplication()
    n = len(idx)
    assert len(M) == n and all(len(row) == n for row in M)
    # Grading diagonal spans -dim/2 to +dim/2 in steps of 1/2
    from fractions import Fraction
    diag = [Gr[a][a] for a in range(n)]
    assert max(diag) - min(diag) == Fraction(X.dimension)


def test_product_a2xa1_sqm_shape():
    """P^2 × P^1 as A2xA1 (node order [1,3]): SQM is square, Gr diagonal."""
    X = FlagVariety('A2xA1', [1, 3])   # P^2 × P^1
    assert X.dimension == 3
    assert len(X.classes) == 6
    M, Gr, idx = X.small_quantum_multiplication()
    n = len(idx)
    assert len(M) == n and all(len(row) == n for row in M)
    # Eigenvalues at y=1 should match P^2×P^1 spectrum (6 eigenvalues)
    ev = X.eigenvalues(M, at_one=True)
    assert len(ev) == 6


def test_product_gr_a2xa2_schubert():
    """Regression: A2xA2 product (P^2 × P^2) uses schubert correctly via
    the localization engine. Verify basic geometric properties and SQM shape."""
    X = FlagVariety('A2xA2', [1, 3])   # P^2 × P^2 (kept nodes 1 and 3)
    assert X.dimension == 4
    assert len(X.classes) == 9          # 3 × 3
    # SQM should produce a 9x9 matrix
    M, Gr, idx = X.small_quantum_multiplication()
    n = len(idx)
    assert len(M) == n and all(len(row) == n for row in M)
    assert n == 9
